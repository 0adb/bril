import * as bril2 from './bril2-ts/bril2.ts';
import {readStdin, unreachable} from './bril2-ts/util.ts';

/**
 * An interpreter error to print to the console.
 */
class Bril2iError extends Error {
  constructor(message?: string) {
    super(message);
    Object.setPrototypeOf(this, new.target.prototype);
    this.name = Bril2iError.name;
  }
}
 ...
 */
function error(message: string): Bril2iError {
  return new Bril2iError(message);
}
...
const argCounts: {[key in bril2.OpCode]: number | null} = {
  add: 2,
  mul: 2,
  sub: 2,
  div: 2,
...
type Env = Map<bril2.Ident, Value>;
type PhiEnv = Map<bril2.Ident, Value>;

/**
 * Check whether a run-time value matches the given static type.
 */
function typeCheck(val: Value, typ: bril2.Type): boolean {
  if (typ === "int") {
 ...
 */
function typeCmp(lhs: bril2.Type, rhs: bril2.Type): boolean {
...
function get(env: Env, ident: bril2.Ident) {
...
function findFunc(func: bril2.Ident, funcs: readonly bril2.Function[]) {
  let matches = funcs.filter(function (f: bril2.Function) {
    return f.name === func;
  });
...
function alloc(ptrType: bril2.ParamType, amt:number, heap:Heap<Value>): Pointer {
 ...
 */
function checkArgs(instr: bril2.Operation, count: number) {
...
function getPtr(instr: bril2.Operation, env: Env, index: number): Pointer {
...
function getArgument(instr: bril2.Operation, env: Env, index: number, typ?: bril.Type) {
  let args = instr.args || [];
...
function getInt(instr: bril2.Operation, env: Env, index: number): bigint {
  return getArgument(instr, env, index, 'int') as bigint;
}

function getBool(instr: bril2.Operation, env: Env, index: number): boolean {
  return getArgument(instr, env, index, 'bool') as boolean;
}

function getFloat(instr: bril2.Operation, env: Env, index: number): number {
  return getArgument(instr, env, index, 'float') as number;
}

function getChar(instr: bril2.Operation, env: Env, index: number): string {
  return getArgument(instr, env, index, 'char') as string;
}

function getLabel(instr: bril2.Operation, index: number): bril2.Ident {
  if (!instr.labels) {
...
function getFunc(instr: bril2.Operation, index: number): bril2.Ident {
  if (!instr.funcs) {
...
type Action =
  ...
  {"action": "jump", "label": bril2.Ident} |
  {"action": "end", "ret": Value | null} |
  {"action": "speculate"} |
  {"action": "commit"} |
  {"action": "abort", "label": bril2.Ident};
...
type State = {
  ...
  readonly funcs: readonly bril2.Function[],
 ...
 */
function evalCall(instr: bril2.Operation, state: State): Action {
 ...
 */
function evalInstr(instr: bril2.Instruction, state: State): Action {
...
function evalFunc(func: bril2.Function, state: State): Value | null {
    ...
    if ('op' in line) {
      ...
      case 'abort': {
        ...
        Object.assign(state, {
          ...
          phiEnv: state.specparent.phiEnv,
      ...
      state.phiEnv = new Map(state.env.entries());
    }
  }
...
function parseMainArguments(expected: bril2.Argument[], args: string[]) : Env {
  let newEnv: Env = new Map();
...
function evalProg(prog: bril2.Program) {
  ...
  let newPhiEnv = new Map(newEnv.entries());
...
async function main() {
  try {
    let prog = JSON.parse(await readStdin()) as bril2.Program;
    evalProg(prog);
  }
  catch(e) {
    if (e instanceof Bril2iError) {
